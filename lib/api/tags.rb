module API
  class Tags < Grape::API
    ...
    resource :projects, requirements: { id: %r{[^/]+} } do
      ...
      desc 'Upload release asset' do
        success Entities::Release
      end
      params do
        requires :tag_name,    type: String, desc: 'The name of the tag'
        optional :file, type: File, desc: %q(Asset's file)
        optional 'file.path', type: String, desc: %q(path to locally stored body (generated by Workhorse))
        optional 'file.name', type: String, desc: %q(real filename as send in Content-Disposition (generated by Workhorse))
        optional 'file.type', type: String, desc: %q(real content type as send in Content-Type (generated by Workhorse))
      end
      post ':id/repository/tags/:tag_name/release/asset', requirements: { tag_name: /.+/ } do
        authorize_push_project
        release = user_project.releases.find_by(tag: params[:tag_name])
        not_found!("release") unless release

        file = uploaded_file(:file, ArtifactUploader.artifacts_upload_path)
        bad_request!('Missing file!') unless file

        asset = release.assets.build(
          project: user_project,
          file: file,
        )

        if asset.save
          present asset, with: Entities::ReleaseAsset
        else
