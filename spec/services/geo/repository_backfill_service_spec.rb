...
describe Geo::RepositoryBackfillService, services: true do
  ...
  describe '#execute' do
    before do
      allow_any_instance_of(described_class).to receive(:geo_primary_project_ssh_url).and_return('git@localhost:/')
      allow(project.repository).to receive(:fetch_geo_mirror)
    end

    context 'when no lease requested for this project' do
    ...
    context 'when a lease exists for this project' do
      before do
        subject.send(:try_obtain_lease)
      end

      it 'does not execute a mirror fetch' do
        expect(project.repository).not_to receive(:fetch_geo_mirror)
