      ...
      module ReadOnly
        module Controller
          ...
          def whitelisted_routes
            grack_route || ::Gitlab::Middleware::ReadOnly.internal_routes.any? { |path| request.path.include?(path) } || lfs_route || sidekiq_route || geo_node_update_route
          end
          ...
          def geo_node_update_route
            ...
            ::Gitlab::Database.db_read_write? &&
              route_hash[:controller] == 'admin/geo_nodes' &&
              route_hash[:action] == 'update'
          end
        end
      end
    end
