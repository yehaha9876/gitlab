...
import { STATUS_RUNNING, STATUS_STARTING, STATUS_STOPPED } from '../../constants';
...
export default {
  ...
  computed: {
    ...
    actionButton() {
      ...
      } else if (status === STATUS_STARTING || status === STATUS_RUNNING) {
        return {
...
<template>
  <div class="ide-terminal build-page d-flex flex-column">
    ...
    <terminal :status="session.status" :terminal-build-path="session.terminalPath" />
  </div>
</template>
