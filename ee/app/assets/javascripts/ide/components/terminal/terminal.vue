...
export default {
  ...
  computed: {
    ...
    wsTerminalPath() {
      return this.terminalBuildPath && `${this.terminalBuildPath}/terminal.ws`;
    },
    isStarting() {
  ...
  methods: {
    recreateTerminal() {
      ...
      $(this.$refs.terminal).find('.xterm-viewport').scroll(event => {
    ...
    onScrollUpdate: _.throttle(
      function throttledOnScrollUpdate({ scrollTop, offsetHeight, scrollHeight }) {
