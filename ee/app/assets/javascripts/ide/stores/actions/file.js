...
import { findEntry, setPageTitle, createTemp, findIndexOfFile } from '../utils';
  ...
  return new Promise((resolve, reject) => {
    service
      ...
      .then(raw => {
        ...
        if (file.mrDiff) {
          service.getRawTargetFileData(file).then(targetRaw => {
            const patchObj = parsePatch(file.mrDiff);
...
export const createTempFile = (
  ...
  if (findEntry(parent.tree, 'blob', file.name))
    return flash(
      `The name "${file.name}" is already taken in this directory.`,
      'alert',
      document,
      null,
      false,
