...
class Groups::OmniauthCallbacksController < OmniauthCallbacksController
  ...
  override :fail_login
  def fail_login(user)
    # No linked GitLab account found
    notice = "Login to a GitLab account to link with your SAML identity"

    #TODO: Token not needed becuase a private group's /sso page
    #       can still be viewed once signed in. Despite this we
    #       should add a test to ensure this continues to work.
    #       Both controller/feature and a QA test with private group.
    #TODO: Should we make token required for private groups even when signed in?
