class Groups::Epics::NotesController < Groups::ApplicationController
  ...
  def epic
    @epic ||= @group.epics.find_by(iid: params[:epic_id] || params[:id])
  ...
  def finder_params
    params.merge(last_fetched_at: last_fetched_at, target_id: params[:epic_id], target_type: 'epic', group_id: @group.id)
  end
