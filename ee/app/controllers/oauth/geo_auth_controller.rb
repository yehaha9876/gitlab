class Oauth::GeoAuthController < ActionController::Base
  ...
  def after_sign_in_with_gitlab(token)
    ...
    redirect_to(oauth.get_oauth_state_return_to.presence || root_path)
  end

  def after_sign_out_with_gitlab
    return_to = oauth.get_logout_state_return_to if oauth.logout_state_valid?
